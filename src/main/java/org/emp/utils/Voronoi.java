...
public class Voronoi
{
    ...
    // Added by Anlan Zhang, 07262020
    public void printAllSites() {
        for (Site site : sites) {
            System.out.println(site.sitenbr + ": " + site.coord.x + ", " + site.coord.y);
        }
    }

    // Added by Anlan Zhang, 07262020
    public void printAllGraphEdges() {
    ...
    // noted that the index of site is equal to its coord's index in xValuesIn/yValuesIn
    public List<int[]> getAllNeighborPairs() {
        List<int[]> allNeighborPairs = new ArrayList<int[]>();
        for (GraphEdge edge : allEdges) {
            int[] pair = {edge.site1, edge.site2};
            allNeighborPairs.add(pair);
        }
        return allNeighborPairs;
    }

    // commented by Anlan Zhang, 07262020
